...
/**
 * Load configuration from CLI args > env vars > config file > defaults
 */
export function loadConfig(argv, defaults = {}) {
  ...
  const rawPort = argv.port ?? process.env.ONVIF_PORT ?? fileConfig.port ?? defaults.port ?? 2020;
  ...
  return {
    ip: argv.ip || process.env.IP_ADDRESS || fileConfig.ip || defaults.ip,
    username: argv.username || process.env.USERNAME || fileConfig.username || defaults.username,
    password: argv.password || process.env.PASSWORD || fileConfig.password || defaults.password,
    port,
  };
}
